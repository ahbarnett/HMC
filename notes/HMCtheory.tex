...
\subsection{M-H for proposals that are measures}
...
We now instead use less abstract notation to cover our needs.

We need M-H where the proposal is a general measure $Q(x,dy)$ which may not
...
This can be summarized by an equality of
...
Using measures, the rejection probability is, for each $x\in S$,
\be
...
\be
K(x,dy) = Q(x,dy) \al(x,y) + r(x) \delta_x(y) dy~,
\label{mhkerm}
\ee
or (Kennedy (8.7)),
\be
...
\section{M-H with symmetric and deterministic proposals}

In our applications the target density $\pi$ as AC.
Consider M-H with non-AC proposal distribution $Q(x,dy)$.
In that case $q(x,y)$ is not a function, and the $\al$ ratio condition
\eqref{alrat} is meaningless.

%However, if $Q$ is symmetric,

We need to define a proposal measure generated by a deterministic smooth map
$F: S\to S$.
This is simply a unit point mass at the image of $x$ under $F$, ie
$$
Q_F(x,B) = 1_{F(x)\in B}
$$
%*** need check jacobean when write as set
or
$$
Q_F(x,dy) = \delta_{F(x)}(y) dy = \delta(y - F(x)) dy ~.
$$
The 
