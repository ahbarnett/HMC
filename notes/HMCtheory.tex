...
$F_2 = L_{\eps/a}^n P$ for some stepsize $a>1$ times smaller;
...
part of the integral from \eqref{dr} gives the more general condition
$$
...
\nonumber \\
&=& \min\left\{ \frac{\max[\pi(F_2(x)) - \pi(F_1(F_2(x)))]}{\pi(x) - \pi(F_1(x))}, 1\right\}~.
\label{al2hmc}
  \eea
The middle argument of $\al_2$, being deterministically given by the first argument, is slightly redundant.
The rule involves four densities to check: $x$, $F_1(x)$, $F_2(x)$, and $F_1(F_2(x))$ a new ghost (pre)image of the 2nd proposal.
